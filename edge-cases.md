...
- **The reconciler is single-session; restart is unsafe.** Group state (union-find, per-group members, next-id) lives in memory and is not rehydrated from the durable DB on startup. So a restart against a populated DB corrupts data on the first write (id collisions overwrite groups; a merge rebuilds a canonical from only the new record). Fix = load groups from the audit table on startup (deferred). The DB access itself is guarded by a lock, so *concurrent* requests within one process are safe.
- **The conflicting-domain veto is scoped to exact name+address only.** If a pair has a *strong-but-fuzzy* name+address (e.g. name 1.0, address 0.95) plus two genuinely different present domains, the domain is merely excluded from the score and the veto does not fire, so the pair can merge on name+address despite conflicting domains. We keep the veto narrow deliberately (decision #10 — we distrust fuzzy-domain reasoning outside the exact branch), and accept this as a residual; broadening it would cut more false merges at the cost of splitting genuine rebrands.
